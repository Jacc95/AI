...
def AS(goal, start, rows, last): #BEST FIRST SEARCH
    ...
    while aux<15:
        ...
        #Checks if one of the surrounding nodes is goal node, then generates path 
        for i in range(len(exp)):
            if (exp[i][0],exp[i][1]) == goal:
                newRoad.append(exp[i])                                                      #Adds goal node to the path
                while (newRoad[0][2], newRoad[0][3]) != start:                              #Condition to know if the path has been fully generated
                    for j in range(len(visited)):              
                        if [newRoad[0][2], newRoad[0][3]] == [visited[j][0],visited[j][1]]: #If the parent has already been visited, add it to the path
                            newRoad.insert(0, visited[j])
                break

        #Kills the While True when the path has been generated by the previous block
        if len(newRoad) != 0:
